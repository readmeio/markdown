...
1. **`terminateHtmlFlowBlocks`**
   Inserts blank lines after standalone HTML elements (like `<div>...</div>`) when the next line is regular markdown. CommonMark's HTML flow rules (types 6/7) only terminate on blank lines, so without this, the parser would swallow subsequent markdown content into the HTML block token.
