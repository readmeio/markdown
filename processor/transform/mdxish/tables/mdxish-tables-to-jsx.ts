...
const containsLiteralNewline = (nodes: Node[]): boolean =>
  nodes.some(
    (node) =>
      (isLiteral(node) && node.value.includes("\n")) ||
...
const mdxishTablesToJsx = (): Transform => (tree) => {
  visit(
    ...
    (table: Table, index, parent) => {
      ...
      // 3. The table does not have column widths, because we want to preserve the original table structure
      ...
      const headerStyles =
        table.align?.map((align, i) => cellStyle(align, widths[i] ?? null)) ?? [];
