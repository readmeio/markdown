...
import type { Code, Construct, Effects, Resolver, State, TokenizeContext } from 'micromark-util-types';
...
// The three tokenizer variants share this factory:
//   - `flow`         : block MDX components (PascalCase always; lowercase only
//                      with a `{…}` attribute). The original mdxComponent contract.
//   - `text`         : inline MDX components inside paragraphs (brace-gated).
//   - `plainHtmlFlow`: plain lowercase HTML block tags (no `{…}` attribute), so
//                      blank lines between nested siblings don't fragment the block.
//                      Lives in its own `plainHtmlBlock` construct; see
//                      `../plain-html-block/syntax.ts`.
export type MdxComponentVariant = 'flow' | 'plainHtmlFlow' | 'text';
...
// Lowercase type-6 block tags the `plainHtmlFlow` variant may claim without a
// `{…}` attribute. Excludes table tags (mdxishTables owns their blank lines) and
// voids (never close).
export const plainBlockClaimTagNames = new Set(
...
/**
 * Builds the `resolveTo` that reattaches a preceding `linePrefix` (indentation)
 * to the component/block token so its start position covers the indent.
 * Parameterized by token type so both the `mdxComponent` and `plainHtmlBlock`
 * constructs can share it.
 */
export function createResolveToLinePrefix(tokenType: 'mdxComponent' | 'plainHtmlBlock'): Resolver {
  return function resolveTo(events: Parameters<Resolver>[0]) {
    let index = events.length;

    while (index > 0) {
      index -= 1;
      if (events[index][0] === 'enter' && events[index][1].type === tokenType) {
        break;
      }
    }
...
/**
 * Factory for the flow (block), text (inline), and plainHtmlFlow variants.
 *
 * **Flow** — claims PascalCase components (always) and lowercase HTML tags that
 * carry at least one `{…}` attribute expression. Multi-line, concrete,
 * `afterClose` consumes the rest of the line.
 *
 ...
 * flow-only. Aborts on line endings and exits after `</tag>` so the paragraph's
 * inline parser picks up the trailing text.
 *
 * **PlainHtmlFlow** — claims plain lowercase block tags (in `plainBlockClaimTagNames`,
 * no brace attribute) so blank lines between nested siblings don't fragment the
 * wrapper. Refuses PascalCase openers, nested table tags, brace attributes,
 * self-closing tags, and markdown islands after a blank line, so every one of
 * those falls back to CommonMark exactly as before this variant existed.
 */
export function createTokenize(variant: MdxComponentVariant) {
  const isFlow = variant !== 'text';
  const isPlainHtmlBlock = variant === 'plainHtmlFlow';
  const tokenType = isPlainHtmlBlock ? 'plainHtmlBlock' : 'mdxComponent';
  const dataTokenType = isPlainHtmlBlock ? 'plainHtmlBlockData' : 'mdxComponentData';
  ...
  return function tokenize(this: TokenizeContext, effects: Effects, ok: State, nok: State) {
    ...
    // `plainHtmlFlow` only: after a blank line the block may continue solely on a
    // markup-only line (gated by `plainClaimLineStart`).
    ...
    function tagNameFirst(code: Code): State | undefined {
      ...
      if (code !== null && code >= codes.uppercaseA && code <= codes.uppercaseZ) {
        // The plain-HTML variant only claims lowercase tags; a component here is
        // mdxComponent's job (it is tried first).
        if (isPlainHtmlBlock) return nok(code);
    ...
    function tagNameRest(code: Code): State | undefined {
      ...
      if (isPlainHtmlBlock) {
        // Only known block tag names, and only when the name ends at a real tag
        // boundary. This rejects custom elements like `<section-header>` (which
        // would otherwise claim as `section` and trigger a doomed scan).
        if (!plainBlockClaimTagNames.has(tagName)) return nok(code);
        if (
          code !== null &&
        ...
        // Lowercase tags are always candidates (brace-gated later in
    ...
    function afterOpenTagName(code: Code): State | undefined {
      ...
      // CommonMark. The plainHtmlFlow variant is the deliberate exception.
      ...
      if (code === codes.greaterThan) {
        // Plain HTML block claims don't need a brace attribute; every other
        // brace-gated variant falls through to CommonMark without one.
        if (requiresBraceAttr && !sawBraceAttr && !isPlainHtmlBlock) return nok(code);
        effects.consume(code);
        onOpenerLine = isFlow;
        return body;
      }
      ...
      if (code === codes.leftCurlyBrace) {
        // A `{…}` attribute means real MDX; leave it to mdxComponent.
        if (isPlainHtmlBlock) return nok(code);
        braceDepth = 1;
        sawBraceAttr = true;
        effects.consume(code);
    ...
    function bodyLessThan(code: Code): State | undefined {
      ...
      // A plain HTML block must not swallow a nested component. Refuse so the
      // whole claim reparses: mdxComponent (tried first) then picks up the
      // component and mdx-blocks can promote it — matching pre-variant behavior.
      if (isPlainHtmlBlock && code !== null && code >= codes.uppercaseA && code <= codes.uppercaseZ) {
        return nok(code);
      }
    ...
    function nestedOpenTagName(code: Code): State | undefined {
      ...
      // A nested table structural tag belongs to mdxishTables, not a plain wrapper
      // claim; refuse so the table's blank lines / cell markdown are handled there.
      if (isPlainHtmlBlock && HTML_TABLE_STRUCTURE_TAGS.has(closingTagName)) {
        return nok(code);
      }
    ...
    function bodyContinuationBefore(code: Code): State | undefined {
      if (code === null || markdownLineEnding(code)) {
        ...
        if (isPlainHtmlBlock && braceDepth === 0) pendingBlankLine = true;
        return bodyContinuationStart(code);
      }
      effects.enter(dataTokenType);
      ...
      if (isPlainHtmlBlock) return plainClaimLineStart(code);
